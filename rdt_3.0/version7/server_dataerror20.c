...
int main(int argc, char *argv[])
{
	...
	newfp = fopen("happy_home20.jpg","wb"); // open the new file in write mode//
	...
	while(received<packets+1)
	{
		...
		if (received == packets) {
			...
			//execution of the code below
               ...
               num = 100; //check the random number generated to see if data has been lost or not
               //let there be no data loss in this case

		if(num>=5)  //if the data loss has not occured ,then consider the packet,else ignore it
	       {
		...
		{	
			...
			j++;
		     ...
		     temp_rcv = rand_corrupt(data_content,bytes); //randomly corrupt the extracted data to simulate data error

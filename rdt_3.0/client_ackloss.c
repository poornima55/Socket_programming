...
int main(int argc, int *argv[])
{
	...
	struct timeval time_start,time_end;  //to measure the timer value for receiving ACK
        int time_diff;
	int rand_num;
		...
		if(nread>0)
		{
			...
			while(1)      //wait for the acknowledgement for the sent packet
			{
			    ...
			    gettimeofday(&time_end,NULL);
                            ...
                            time_diff=((time_end.tv_sec - time_start.tv_sec)*1000000u+ time_end.tv_usec - time_start.tv_usec)/1.e6;


			    if(time_diff<0.05)    //if it is not time out ACK has reached well in time
		            {
			...
			else  //even if the sequence number of the ack packet does not match then resend the data packet to the server
			{
			    ...
			    else     //ACK has not reached and time is out, resend the packet

			  {

				  printf("timeout happened and the packet is being sent again\n");
				  n = sendto(sock,data,total_len,0,(struct sockaddr *)&server,sizeof(struct sockaddr));

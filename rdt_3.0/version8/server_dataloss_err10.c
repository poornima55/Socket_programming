...
int main(int argc, char *argv[])
{
	...
	newfp = fopen("new_home10.jpg","wb"); // open the new file in write mode//
	...
	while(received<packets+1)
	{
		...
		if (received == packets) {
			...
			//execution of the code below
               ...
               num = rand()%100; //check the random number generated to see if data has been lost or not
               //let there be 10% data loss

		if(num>=10)  //if the data loss has not occured ,then consider the packet,else ignore it
	       {

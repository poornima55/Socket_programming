...
//	ack_buff = calloc(1,256); //allocating memory for acknowledgement from the server
	...
	while(1)
	{
		...
		if(nread>0)
		{
			...
			strcat(data,bytes_len); //adding data length
			strcat(data,bytes_info); //adding the data length info
	                strcat(data,buf);   //adding data packets		
			//printf("the new  data is %s\n",new_buff);
                        total_len = nread+3+check_length;//finding the total length

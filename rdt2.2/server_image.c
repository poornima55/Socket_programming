...
char* rand_corrupt(char* buff,unsigned short int bytes)
{
	...
	char* str_temp =NULL;
        str_temp =(char*)calloc(1,bytes); //allocate memory for str_temp 
	int i;
	int num;

	num = rand() %100;            //generate a random number

	if(num<5)                      //5 % loss/bit error
	{
              for(i=0;i<bytes;i++)
	      {
		      buff[i] = buff[i]+1;
...
int main(int argc, char *argv[])
{
	...
	newfp = fopen("sweet_home.jpg","wb"); // open the new file in write mode//
	...
	while(received<packets)
	{
		...
		//memcpy(seq_no,buf,sizeof(seq_no));
		...
		{	
		     ...
		     for(i=1;i<2;i++)              //the next byte will have the length of the checksum of the received packet
		      ...
		      data_check_len[j] = '\0';    //making it a string by appending a null at the end 
                     // memcpy(data_check_len,data[1],1);
		      //data_check_len[1] = '\0';
                    // printf("the checksum length in string form is %s\n",data_check_len);
			...
			data_checksum[j] = buf[i];  //extracting them in the string form
		     ...
		     //data_checksum[j] = '\0';  //ending the string by appending a NULL character //removr?

                     //memcpy(data_checksum,data[2],data_checksum_len);
                     //data_checksum[data_checksum_len] = '\0';

		     data_checksum_value = atoi(data_checksum);	//converting the checksum value to interger form
		     printf("the data_checksum_value is %d\n",data_checksum_value);

		     //i = 2+data_checksum_len;
                     
                     data_len_info[0]=buf[i];
                     data_len_info[1] = '\0';

		     //printf("the length of data is %s\n", data_len_info);

		     data_len = atoi(data_len_info);
		     i++;
		     //printf("the length of data in integer form is %d\n",data_len);
                     
		     //memcpy(data_len_info,data[i],1);
		     //data_len_info[1] = '\0';
		     //data_len = atoi(data_len_info);
		     ...
		     //memcpy(bytes_length,data[i],data_len);
		     //bytes_length[data_len] = '\0';
			...
			j++;
		     ...
		     //data_content[j] ='\0';  //ending the string with a null character //remove later?

                     //printf("the data extracted is %s\n",data_content);

                     //memcpy(data_content,data[i],bytes);
		     data_content[bytes] = '\0';


		     temp_rcv = rand_corrupt(data_content,bytes);

                     memcpy(data_rcv,temp_rcv,bytes);            
           
		    // len=strlen(data_content); //length of the string
		    // printf ("length of the data content is %d\n", len);

	             real_checksum = checksum(data_rcv,bytes); //calculating the actual checksum of the received packet

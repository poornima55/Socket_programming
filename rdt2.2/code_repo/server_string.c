...
int main(int argc, char *argv[])
{
	...
	char data_content[65535]; //the actual payload information ie the data contained in the packet
	...
	while(received<packets)
	{
		...
		//printf("the real checksum is %d\n",real_checksum);
			...
			strcat(reply,new_ack); //append the payload data at the end
		        //printf("reply is %s\n",reply);	
                        printf("Ack sent to the packet %c\n",seq_no);

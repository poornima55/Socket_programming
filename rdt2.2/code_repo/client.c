...
//	ack_buff = calloc(1,256); //allocating memory for acknowledgement from the server
	...
	while(1)
	{
		...
		if(nread>0)
		{
			 ...
			 length1 = strlen(buff); //calculate the length  of the buff 
            		 check = checksum(buff,length1);    //calculate the checksum
    			 itoa(check,checksum_info,10);  //convert checksum into string
			 check_length = strlen(checksum_info)-1;  //calculate the length of the checksum
			    ...
			    if(seq_no == current)    //if the sequence number matches then proceed 
		               {
			                ...
			                j =0;
			                for(i=1;i<=4;i++)   //next 4  bytes will have the length of checksum
		                      {

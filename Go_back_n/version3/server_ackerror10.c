...
int main(int argc, char *argv[])
{
	...
	newfp = fopen("sejal.jpg","wb"); // open the new file in write mode//
	...
	while(received<packets+1)
	{
		...
		else
		{
			...
			itoa(ack_seq,seq_char,10); //form the reply, append the sequence number of the last received packet
	...
	//if the packet received does not have expected seqno then send ack to the last received packet ok
		else
		{
			...
			printf("so the ack_seq is still %d\t only",ack_seq);
			...
			itoa(ack_seq_len,ack_seqlen_char,10); //convert the sequence number length to string
		        ...
		        strcat(reply,ack_seqlen_char); //append the sequence number length to the reply
		        strcat(reply,seq_char);
                        printf("%s is the sequence number length\n",ack_seqlen_char);
			printf("%s is the sequence number\n",seq_char);

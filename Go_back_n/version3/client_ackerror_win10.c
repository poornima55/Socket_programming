...
int num; //variable for error rate
...
main (int argc, char **argv)
{
	...
	printf("Enter the ack  error rate\n");
	scanf("%d",&num);
	...
	int N =10; //window size is fixed to 10 in this case
	...
	itoa(N,(char*)buff,10);
	printf("window size = %s",(char*)buff);
	...
	while(1)
	{
	        ...
	        while(1)      //wait for the acknowledgement for the sent packets till timeout , here timer value is set to 50 ms
		{
			    ...
			    msec = difference*1000/CLOCKS_PER_SEC;
				 ...
				 //printf ("Receiving packet\n");
					...
					seq_no = atoi(ack_seq_char);
					 ...
					 temp_ack_rcv = rand_corrupt(content,length1);
			  ...
			  sequence =  base;
			  printf("packet sent after timeout is %d\t",sequence);
                          while(sequence <= nextseqnum-1)
		          {
					...
					for(i=1;i<1+ack_seqlen;i++)
					{
			    		       ack_seq_char[j]= data[sequence][i]; 
					       j++;	//next bytes till i=ack_seqlen will  be sequence number
					}
					ack_seq_char[j] = '\0';
                                        seq_no = atoi(ack_seq_char);
		                        ...
		                        printf("Sending packet with the sequence number %d\t",seq_no);		  

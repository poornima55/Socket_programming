...
int main(int argc, char *argv[])
{
	...
	int seq_no;      //sequence number of the current packet
	...
	char* data = NULL;
         ...
         char seqnum_char[10] = {0,};
	 char expectedseq_char[10] ={0,};
         int N= 10;
	...
	while(received<packets+1)
	{
		...
		if (received == packets) {
			...
			//execution of the code below
               ...
               //let there be no loss or error in this case
	        j=0;
		for(i=0;i<1;i++)
		{
                      seqnum_char[j]= buf[i];
		...
		seqnum_char[j] = '\0';
		printf("sequence number present is %s\n",seqnum_char);
		seq_no=(atoi)(seqnum_char);   //extract the sequence number of the packet received

		printf("The sequence number is %d\n",seq_no);
		printf("The expected number is %d\n",expectedseq);
         
	        if(seq_no == expectedseq)   //if the sequence number is equal to the expected one then proceed
		{	
			...
			j++;
		     ...
		     printf("real checksum is %d and checksum in the packet is %d\n",real_checksum,data_checksum_value);
			...
			itoa(ack_check_len,ack_check_len_str,10);//converting the checksum length to string form
                        ...
                        itoa(expectedseq,expectedseq_char,10);
			strcat(reply, expectedseq_char);; //forming the reply packet i.e append  the sequence number
		...
		else
		{
                        ...
                        printf("Ack sent for the last received packet by default\n");
